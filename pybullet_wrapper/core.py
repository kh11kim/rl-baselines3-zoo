...
class Bullet:
    ...
    def set_base_pose(self, body: str, position: np.ndarray, orientation: np.ndarray) -> None:
        ...
        """
        if body in self._bodies_idx.keys():
            idx = self._bodies_idx[body]
        if body in self._ghost_idx.keys():
            idx = self._ghost_idx[body]
        ...
        self.physics_client.resetBasePositionAndOrientation(
            bodyUniqueId=idx, posObj=position, ornObj=orientation
        )
