...
class PandaCollisionEnv(PandaAbstractEnv, gym.GoalEnv):
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        if len(achieved_goal.shape) == 2:
            r = -np.ones(len(info))#np.zeros(len(info))
...
register(
    id='PandaCollision-v0',
    entry_point='utils.rxbot.panda_collision:PandaCollisionEnv',
