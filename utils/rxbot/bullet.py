...
class Bullet:
    ...
    def is_collision(self, bodyA, bodyB, linkA=None, linkB=None):
        """Get closest point from of a link.

        Args:
            bodyA (str): Body unique name.
            bodyB (str): Body unique name.
            linkA (int): Link index in the bodyA. Default is None (no link)
            linkB (int): Link index in the bodyB. Default is None (no link)
        """
        if linkA is None:
            linkA = -1
        if linkB is None:
            linkB = -1
        dist_info = self.physics_client.getClosestPoints(
        ...
        return len(dist_info) != 0

    def view_pos(self, name, pos):
